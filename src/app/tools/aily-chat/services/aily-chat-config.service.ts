...
export class AilyChatConfigService {
    ...
    get models(): ModelConfigOption[] {
        ...
        this.config.models = this.config.models.filter(m => m.isCustom);
    ...
    /**
     * 获取已启用的模型列表
     * 仅允许自定义模型，不再回退到官方模型或 Auto。
     */
    getEnabledModels(): ModelConfigOption[] {
        return this.models.filter(m => m.enabled && m.isCustom && !!m.baseUrl && !!m.apiKey);
    }
    ...
    migrateFromOldConfig(): void {
        ...
        if (this.config.models) {
            this.config.models = this.config.models.filter(model => model.isCustom);
        }

        if (this.config.apiKeys) {
            this.config.apiKeys = this.config.apiKeys.filter(apiKey => !!apiKey.baseUrl && !!apiKey.apiKey);
        }

        const savedChatModel = this.config as AilyChatConfig & { aiChatModel?: ModelConfigOption | null };
        if (savedChatModel.aiChatModel && !savedChatModel.aiChatModel.isCustom) {
            savedChatModel.aiChatModel = null;
        }
    }
}
