...
export class ChatEngineService {
  ...
  private setupSubscriptions(): void {
    ...
    // 无登录模式下也允许初始化会话；若仍存在鉴权状态流，则仅同步状态，不再强制清空会话
    const authLoggedIn$ = AilyHost.get().authFull?.isLoggedIn$;
    if (authLoggedIn$) {
      this.loginStatusSubscription = authLoggedIn$.subscribe(async isLoggedIn => {
        this.isLoggedIn = !!isLoggedIn;
        if (!this.hasInitializedForThisLogin && !this.isSessionStarting) {
          this.bootstrapChatSession();
        }
      });
    } else {
      this.bootstrapChatSession();
    ...
    this.configChangedSubscription = this.ailyChatConfigService.configChanged$.subscribe(
      async (newConfig) => {
        ...
        if (!hasConversationHistory && this.sessionId) {
          try {
  ...
  private bootstrapChatSession(): void {
    if (this.hasInitializedForThisLogin || this.isSessionStarting) {
      return;
    }
